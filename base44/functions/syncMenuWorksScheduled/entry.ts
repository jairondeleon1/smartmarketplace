...
Deno.serve(async (req) => {
  try {
    ...
    // Scheduled automations run without a user session, so we skip the
    // user-auth gate and operate with the service role directly.
    ...
    const options = {
      filter: { startDate, days },
      include: { allergens: true, nutrientTypes: ['Standard'], ingredients: false },
    ...
    const apiRes = await fetch(url, {
      headers: { 'WT-Client-Id': WT_CLIENT_ID || '', 'Accept': 'application/json' }
    });
    ...
    if (menuItems.length === 0) {
      return Response.json({ success: true, synced: 0, message: 'No menu items returned from API', startDate });
    }
    ...
    // Full replacement for this location only
