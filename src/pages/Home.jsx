...
const DEFAULT_MENU = [
  ...
  // FRIDAY (Feb 20)
  { id: 6, station: "Entree", name: 'Shrimp, Sausage & Jambalaya', description: 'A classic Mardi Gras stew with andouille sausage, shrimp, and clams.', ingredients: 'Shrimp, Andouille Sausage, Clams, Rice, Tomato Sauce, Cajun Seasoning, Bell Peppers, Onions.', calories: 480, protein: 32, carbs: 45, fat: 18, saturated_fat: 6, unsaturated_fat: 12, sodium: 980, fiber: 3, sugar: 4, cholesterol: 185, vitamin_a: 420, vitamin_c: 38, vitamin_d: 2.8, calcium: 180, iron: 5.2, potassium: 680, tags: ['High Protein', 'Spicy', 'Dairy Free', 'Avoid Gluten', 'Fit'], allergens: ['Shellfish', 'Soy'], day: 'Friday' },
  { id: 601, station: "Entree", name: 'Cheese Tortellini', description: 'Cheese filled tortellini served with marinara sauce.', ingredients: 'Cheese Tortellini (Wheat, Egg, Milk), Marinara Sauce, Parmesan Cheese, Basil.', calories: 320, protein: 12, carbs: 48, fat: 8, saturated_fat: 4, unsaturated_fat: 4, sodium: 450, fiber: 3, sugar: 6, cholesterol: 42, vitamin_a: 280, vitamin_c: 15, vitamin_d: 0.8, calcium: 240, iron: 2.4, potassium: 320, tags: ['Vegetarian'], allergens: ['Milk', 'Wheat', 'Egg'], day: 'Friday' },
          ...
          {item.ingredients && (
            <div className="bg-teal-50 border border-teal-100 rounded-xl p-4 mb-3">
              ...
              {item.ingredients.includes('^') && (
                <p className="text-[10px] text-teal-700 mt-2 italic">^ These items contain additional ingredients. If you have questions, please ask an Ingredient Ambassador.</p>
              )}
            </div>
          )}
...
function getMealPeriodFromStation(item) {
  ...
  if (station.startsWith('lunch') || station.startsWith('main') || station.startsWith('entree') || station.startsWith('soup') || station.startsWith('pizza') || station.startsWith('deli') || station.startsWith('grill') || station.startsWith('dessert')) return 'Lunch';
...
export default function Home() {
  ...
  const filteredItems = menuItems.map(item => ({ ...item, station: normalizeStation(item.station) })).filter(item => {
    ...
    const grainKeywords = ['whole wheat', 'brown rice', 'oats', 'oatmeal', 'quinoa', 'farro', 'barley', 'bulgur', 'couscous', 'millet', 'whole grain', 'rye', 'cornmeal', 'grain'];
    const grainText = ((item.ingredients || '') + ' ' + (item.name || '')).toLowerCase();
    if (grainKeywords.some(k => grainText.includes(k)) && !autoTags.includes('Whole Grain')) autoTags.push('Whole Grain');
